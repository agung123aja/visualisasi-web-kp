...
app = Flask(__name__)
app.secret_key = 'kunci_super_rahasia_dan_aman_milik_anda_12345'
...
# Pastikan nama-nama ini konsisten dengan nama_bagian di realisasi_per_bagian
# dan bagaimana mereka dipetakan ke nama_kategori di dppa_target_per_kategori
nama_bagian_to_kategori = {
    ...
    'WILAYAH III PURWAKARTA': 'purwakarta', # <-- Perbaikan spelling: 'purwakart' menjadi 'purwakarta'
...
def process_reports():
    ...
    try:
        ...
        # Penting: Pastikan nama_kategori di database sesuai dengan .title() dari kategori_dppa_target
        # Cek database, misalnya 'Sekretariat' vs 'sekretariat'
        ...
        # Kunci di dppa_target_dict harus sesuai dengan yang dihasilkan oleh kategori_short.replace('_', ' ').title()
        ...
        hasil_perhitungan = []
        # Gunakan list nama_bagian_to_kategori.keys() yang lengkap untuk iterasi
        for nama_bagian_db_key in nama_bagian_to_kategori.keys():
            nama_bagian_display = nama_bagian_db_key # Nama bagian untuk ditampilkan/digunakan di realisasi_akumulatif_dict
            kategori_dppa_key = nama_bagian_to_kategori.get(nama_bagian_db_key) # Kunci kategori singkat

            realisasi = realisasi_akumulatif_dict.get(nama_bagian_display, 0.0)

            # Ambil info DPPA/Target. Pastikan nama_kategori di DB cocok dengan format title-cased
            # Misalnya, jika kategori_dppa_key adalah 'sekretariat', maka nama_kategori di DB harus 'Sekretariat'
            dppa_info = dppa_target_dict.get(kategori_dppa_key.replace('_', ' ').title(), {'dppa': 0.0, 'target': 0.0})

            target = dppa_info['target']
            dppa_skpd = dppa_info['dppa']

            # DEBUGGING: Log nilai sebelum perhitungan
            app.logger.info(f"Processing: {nama_bagian_display}")
            app.logger.info(f"  Realisasi: {realisasi}")
            app.logger.info(f"  Target DPPA info: {dppa_info}")
            app.logger.info(f"  Target: {target}, DPPA SKPD: {dppa_skpd}")
            ...
            # DEBUGGING: Log hasil perhitungan
            app.logger.info(f"  Calculated dppatarget: {dppatarget}")
            app.logger.info(f"  Calculated dpparealisasi: {dpparealisasi}")
            app.logger.info(f"  Calculated deviasi_bersih: {deviasi_bersih}")


            hasil_perhitungan.append({
                'nama_bagian': nama_bagian_display,
...
def generate_pdf_report():
    try:
        ...
        col_widths = [1*cm, 6*cm, 3*cm, 4*cm, 4*cm]
